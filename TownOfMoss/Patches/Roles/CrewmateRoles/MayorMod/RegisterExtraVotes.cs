...
namespace TownOfUs.CrewmateRoles.MayorMod
{
    ...
    public class RegisterExtraVotes
    {
        ...
        // {
        //     if (!PlayerControl.LocalPlayer.Is(RoleEnum.Mayor) && !PlayerControl.LocalPlayer.Is(RoleEnum.Executioner)) return;
        //     if (PlayerControl.LocalPlayer.Data.IsDead) return;
        //     if (__instance.cooldownTimerText.text.Contains("Can Vote")) return;
        //     var role = Role.GetRole<Mayor>(PlayerControl.LocalPlayer);
        //     __instance.cooldownTimerText.text = "Can Vote: " + role.VoteBank + " time(s) | " + __instance.cooldownTimerText.text;
        // }
        ...
        public static Dictionary<byte, int> CalculateAllVotes(MeetingHud __instance)
        {
            ...
            foreach (var role in Role.AllRoles.Where(x => x.RoleType == RoleEnum.Mayor)) {
            ...
            if (tie)
                foreach (var player in __instance.playerStates)
                {
                    ...
                    if (Utils.Is(GameData.Instance.GetPlayerById(player.TargetPlayerId).Object, RoleEnum.Mayor))
                    {
        ...
        // {
        //     foreach (var role in Role.AllRoles.Where(x => x.RoleType == RoleEnum.Mayor || x.RoleType == RoleEnum.Executioner))
        //     {
        //         var mayor = (Mayor) role;
        //         AmongUsExtensions.Log($"{mayor.PlayerName} mayor.ExtraVotes.Clear");
